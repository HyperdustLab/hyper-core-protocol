...
async function main() {
  ...
  const _HyperAGI_Office_TaskReward = await ethers.getContractFactory('HyperAGI_Office_TaskReward')
  console.log('Contract factory created successfully')

  const proxyAddress = '0x0E4710191dA974B825d969283f1D141F06cc7576' // TODO: 替换为实际的代理地址
  console.log('Proxy contract address:', proxyAddress)

  const HyperAGI_Office_TaskReward = await upgrades.upgradeProxy(proxyAddress, _HyperAGI_Office_TaskReward)
  console.log('Contract upgraded successfully!')
  console.log('Proxy contract address:', await HyperAGI_Office_TaskReward.getAddress())

  const implementationAddress = await upgrades.erc1967.getImplementationAddress(await HyperAGI_Office_TaskReward.getAddress())
