...
contract HyperAGI_Agent_Wallet is OwnableUpgradeable {
    ...
    function allocateWallet() public payable onlyAdmin returns (address) {
        require(nextWalletIndex < walletAddresses.length, "no available wallets");

        address walletAddress = walletAddresses[nextWalletIndex];

        // Double check: ensure wallet is not already allocated
        if (_isWalletAllocated[walletAddress]) {
            // Find next available wallet
            for (uint256 i = nextWalletIndex + 1; i < walletAddresses.length; i++) {
                if (!_isWalletAllocated[walletAddresses[i]]) {
                    walletAddress = walletAddresses[i];
                    nextWalletIndex = i;
                    break;
                }
            }
            require(!_isWalletAllocated[walletAddress], "no available wallets");
        }
    ...
    // Get next available wallet index (for debugging)
    function getNextAvailableWalletIndex() public view returns (uint256) {
        for (uint256 i = nextWalletIndex; i < walletAddresses.length; i++) {
            if (!_isWalletAllocated[walletAddresses[i]]) {
                return i;
            }
        }
        return walletAddresses.length; // No available wallets
    }

    // Reset wallet allocation status (admin only, for emergency use)
    function resetWalletAllocation(address walletAddress) public onlyAdmin {
        require(_isWalletAllocated[walletAddress], "wallet not allocated");
        _isWalletAllocated[walletAddress] = false;

        // Update nextWalletIndex if this wallet is before current index
        for (uint256 i = 0; i < walletAddresses.length; i++) {
            if (walletAddresses[i] == walletAddress && i < nextWalletIndex) {
                nextWalletIndex = i;
                break;
            }
        }
